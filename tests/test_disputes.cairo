...
func test_dispute2{syscall_ptr: felt*, range_check_ptr, pedersen_ptr: HashBuiltin*}() {
    ...
    IGasLessLiar.open_dispute_state_2(
        ...
        s2,
        h1,
        (state2_sig0, state2_sig1),
        (state1_sig0, state1_sig1),
    );
...
@external
func test_dispute3{syscall_ptr: felt*, range_check_ptr, pedersen_ptr: HashBuiltin*}() {
    alloc_locals;
    local eth_contract;
    local gll_contract;
    %{
    ...
    %{
        ...
        ids.starting_card = pedersen_hash(s1, state2['s2'])
        state3 = { 'game_id' : 1, 'prev_state_hash' : hashed_state2, 's1' : s1, 'starting_card' : ids.starting_card, 'type' : 3}
        ids.hashed_state3 = pedersen_hash(pedersen_hash(pedersen_hash(pedersen_hash(state3['game_id'], state3['prev_state_hash']), state3['s1']), state3['starting_card']), state3['type'])
        ids.state3_sig0, ids.state3_sig1 = sign( ids.hashed_state3, 1)

        # state4
        a0 = random.randrange(P)
        a1 = random.randrange(P)
        a2 = random.randrange(P)
        a3 = random.randrange(P)
        ids.ah0 = pedersen_hash(a0, 0)
    ...
    // let's open and close a dispute 1
    IGasLessLiar.open_dispute_state_3(
        gll_contract,
        'dispute_1',
        1,
        hashed_state1,
